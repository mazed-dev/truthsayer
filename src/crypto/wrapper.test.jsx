...
test('simple enc & openssl dec', () => {
  ...
  const cmd = `echo -n "${encryptedBase64}" | base64 -d | openssl enc -aes-256-cbc -d -pass pass:"${passphrase}" -md md5`
...
test('simple openssl enc & dec', () => {
  ...
  // I don't use -base64 option of openssl itself because of line wrap, there is
  // no clear way to turn it off.
  const cmd = `echo -n "${inputText}" | openssl enc -aes-256-cbc -e -md md5 -pass pass:"${passphrase}" | base64 --wrap 0`
